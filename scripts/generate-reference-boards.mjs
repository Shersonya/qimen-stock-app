...
function computeBoard(datetime) {
  const board = TimeDunjia.create({
    datetime: new Date(datetime.replace(' ', 'T') + ':00+08:00'),
    type: 'hour',
  });

  const valueStarPalace = board.palaces.find(
    (item) => item.god?.name === '值符' && item.star,
  );
  const valueDoorPalace = board.palaces.find(
    (item) =>
      valueStarPalace && item.door?.originPalace === valueStarPalace.star.originPalace,
  );

  return {
    yinYang: board.meta.yinyang,
    ju: board.meta.juNumber,
    valueStar: valueStarPalace?.star?.name ?? '--',
    valueDoor: valueDoorPalace?.door?.name ?? '--',
    palaces: board.palaces.map((item) => ({
      index: item.index,
...
function renderBoardSvg(reference) {
...
for (const board of boards) {
  ...
  await fs.writeFile(path.join(outputDir, `ref-${board.key}.svg`), svg, 'utf8');
}

console.log('Reference SVGs generated.');
